...
function WeatherScreen({ route, navigation }) {
  ...
  if (lat && lon) data = useCurrentAndForecastWeather(lat, lon, city);
  else data = useCurrentAndForecastWeather();
    ...
    <View style={styles.container}>
      {data?.loading ? (
        ...
        <Container>
          ...
          <ForecastCard
            currentWeather={data?.weatherCurrentState}
            forecastWeather={data?.weatherForecast}
          />
        </Container>
      )}
    </View>
